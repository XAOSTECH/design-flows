#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# json-gen.sh — VS Code theme JSON generation for vscode-theme-gen
# ═══════════════════════════════════════════════════════════════════════════════
# Builds the "workbench.colorCustomizations" and
# "editor.tokenColorCustomizations" JSON blocks.
#
# Requires: colours.sh sourced (for pastel helpers + ensure_readable)
# ═══════════════════════════════════════════════════════════════════════════════

generate_theme_json() {
    local primary_hex secondary_hex tertiary_hex
    primary_hex=$(get_hex "$PRIMARY_COLOUR")
    secondary_hex=$(get_hex "$SECONDARY_COLOUR")
    tertiary_hex=$(get_hex "$TERTIARY_COLOUR")

    # ── Background colours (adjusted by BG_LIGHTNESS, tinted with primary) ──
    # BG_LIGHTNESS: 0.0 = pure black, 0.12 = default dark, 0.5 = medium gray, 1.0 = white
    # Convert to 0-100 scale for pastel
    local bg_base bg_main bg_sidebar bg_panel bg_dark bg_hover bg_input bg_widget
    
    # Calculate lightness values (convert 0.0-1.0 to 0-100 for pastel)
    local bg_main_l bg_sidebar_l bg_panel_l bg_dark_l bg_hover_l bg_input_l bg_widget_l
    bg_main_l=$(awk "BEGIN { printf \"%.1f\", $BG_LIGHTNESS * 100 }")
    bg_sidebar_l=$(awk "BEGIN { printf \"%.1f\", ($BG_LIGHTNESS + 0.02) * 100 }")
    bg_panel_l=$(awk "BEGIN { printf \"%.1f\", ($BG_LIGHTNESS + 0.05) * 100 }")
    bg_dark_l=$(awk "BEGIN { l=$BG_LIGHTNESS - 0.02; printf \"%.1f\", ((l > 0) ? l : 0) * 100 }")
    bg_hover_l=$(awk "BEGIN { printf \"%.1f\", ($BG_LIGHTNESS + 0.10) * 100 }")
    bg_input_l=$(awk "BEGIN { printf \"%.1f\", ($BG_LIGHTNESS + 0.07) * 100 }")
    bg_widget_l=$(awk "BEGIN { printf \"%.1f\", ($BG_LIGHTNESS + 0.04) * 100 }")
    
    # Create base grays at target lightness, then apply primary tint
    # Mix factors: higher = more gray (subtle tinting), lower = more primary (stronger tinting)
    bg_main=$(mix_colors "$(set_lightness '#808080' "$bg_main_l")" "$primary_hex" 0.90)
    bg_sidebar=$(mix_colors "$(set_lightness '#808080' "$bg_sidebar_l")" "$primary_hex" 0.88)
    bg_panel=$(mix_colors "$(set_lightness '#808080' "$bg_panel_l")" "$primary_hex" 0.85)
    bg_dark=$(mix_colors "$(set_lightness '#808080' "$bg_dark_l")" "$primary_hex" 0.92)
    bg_hover=$(mix_colors "$(set_lightness '#808080' "$bg_hover_l")" "$primary_hex" 0.82)
    bg_input=$(mix_colors "$(set_lightness '#808080' "$bg_input_l")" "$primary_hex" 0.87)
    bg_widget=$(mix_colors "$(set_lightness '#808080' "$bg_widget_l")" "$primary_hex" 0.86)

    # ── Foreground colours – all passed through ensure_readable ──
    local fg_main fg_soft fg_muted fg_bright
    fg_main=$(ensure_readable "$(lighten "$primary_hex" 0.35)")
    fg_soft=$(ensure_readable "${PRIMARY_SHADES[4]:-$(lighten "$primary_hex" 0.25)}")
    fg_muted=$(ensure_readable "$(lighten "$primary_hex" 0.18)" 0.40)
    fg_bright=$(ensure_readable "$(lighten "$secondary_hex" 0.30)")

    # ── Accent colours ──
    local accent_primary accent_secondary accent_tertiary accent_soft
    accent_primary=$(ensure_readable "${PRIMARY_SHADES[0]:-$primary_hex}")
    accent_secondary=$(ensure_readable "${SECONDARY_SHADES[0]:-$secondary_hex}")
    accent_tertiary=$(ensure_readable "${TERTIARY_SHADES[0]:-$tertiary_hex}")
    accent_soft=$(ensure_readable "${PRIMARY_SHADES[3]:-$(lighten "$primary_hex" 0.1)}")

    # ── Tertiary accent helpers ──
    local tert_soft tert_bright
    tert_soft=$(ensure_readable "${TERTIARY_SHADES[3]:-$(lighten "$tertiary_hex" 0.1)}")
    tert_bright=$(ensure_readable "${TERTIARY_SHADES[1]:-$(lighten "$tertiary_hex" 0.2)}")

    # ── Complementary accents (if enabled) ──
    local compl_highlight="" compl_border="" compl_accent=""
    if [[ "$USE_COMPLEMENTARY" == true ]] && [[ ${#COMPLEMENTARY_COLOURS[@]} -gt 0 ]]; then
        compl_highlight=$(ensure_readable "${COMPLEMENTARY_COLOURS[0]}")
        compl_border="${COMPLEMENTARY_COLOURS[2]}"
        compl_accent=$(ensure_readable "${COMPLEMENTARY_COLOURS[4]}")
    fi

    # ── Badge colours (always visible against dark backgrounds) ──
    local badge_bg badge_fg
    if [[ -n "$compl_highlight" ]]; then
        badge_bg="$compl_highlight"
    else
        badge_bg="$accent_secondary"
    fi
    badge_fg="$bg_dark"

    # ── Analogous & triadic helpers ──
    local ana0 ana1 ana2 ana3 tri0 tri1 bln0 bln1 bln2 bln3
    ana0=$(ensure_readable "${ANALOGOUS_COLOURS[0]:-$accent_primary}")
    ana1=$(ensure_readable "${ANALOGOUS_COLOURS[1]:-$accent_primary}")
    ana2=$(ensure_readable "${ANALOGOUS_COLOURS[2]:-$accent_secondary}")
    ana3=$(ensure_readable "${ANALOGOUS_COLOURS[3]:-$accent_secondary}")
    tri0=$(ensure_readable "${TRIADIC_COLOURS[0]:-$accent_tertiary}")
    tri1=$(ensure_readable "${TRIADIC_COLOURS[1]:-$accent_tertiary}")
    bln0=$(ensure_readable "${BLEND_COLOURS[0]:-$accent_primary}")
    bln1=$(ensure_readable "${BLEND_COLOURS[1]:-$accent_secondary}")
    bln2=$(ensure_readable "${BLEND_COLOURS[2]:-$accent_tertiary}")
    bln3=$(ensure_readable "${BLEND_COLOURS[3]:-$fg_soft}")

    # ── Variation accents ──
    local var0="" var1="" var2="" var3=""
    if [[ ${#VARIATION_COLOURS[@]} -gt 0 ]]; then
        var0=$(ensure_readable "${VARIATION_COLOURS[0]}")
        var1=$(ensure_readable "${VARIATION_COLOURS[1]:-$var0}")
        var2=$(ensure_readable "${VARIATION_COLOURS[2]:-$var0}")
        var3=$(ensure_readable "${VARIATION_COLOURS[3]:-$var0}")
    fi

    # ── Readable shade helpers (used extensively below) ──
    local p1 p2 p3 p4 p5 p6 p7
    p1=$(ensure_readable "${PRIMARY_SHADES[1]:-$accent_primary}")
    p2=$(ensure_readable "${PRIMARY_SHADES[2]:-$accent_soft}")
    p3=$(ensure_readable "${PRIMARY_SHADES[3]:-$accent_soft}")
    p4=$(ensure_readable "${PRIMARY_SHADES[4]:-$fg_soft}")
    p5=$(ensure_readable "${PRIMARY_SHADES[5]:-$fg_soft}")
    p6=$(ensure_readable "${PRIMARY_SHADES[6]:-$fg_main}")
    p7=$(ensure_readable "${PRIMARY_SHADES[7]:-$fg_main}")

    local s1 s2 s3 s4 s5 s6 s7
    s1=$(ensure_readable "${SECONDARY_SHADES[1]:-$accent_secondary}")
    s2=$(ensure_readable "${SECONDARY_SHADES[2]:-$accent_secondary}")
    s3=$(ensure_readable "${SECONDARY_SHADES[3]:-$accent_secondary}")
    s4=$(ensure_readable "${SECONDARY_SHADES[4]:-$fg_bright}")
    s5=$(ensure_readable "${SECONDARY_SHADES[5]:-$fg_main}")
    s6=$(ensure_readable "${SECONDARY_SHADES[6]:-$fg_main}")
    s7=$(ensure_readable "${SECONDARY_SHADES[7]:-$fg_main}")

    local t1 t2 t3 t4 t5
    t1=$(ensure_readable "${TERTIARY_SHADES[1]:-$accent_tertiary}")
    t2=$(ensure_readable "${TERTIARY_SHADES[2]:-$accent_tertiary}")
    t3=$(ensure_readable "${TERTIARY_SHADES[3]:-$tert_soft}")
    t4=$(ensure_readable "${TERTIARY_SHADES[4]:-$tert_soft}")
    t5=$(ensure_readable "${TERTIARY_SHADES[5]:-$tert_bright}")

    # ── Emit JSON ─────────────────────────────────────────────────────────────
    cat << THEME_EOF
		"workbench.colorCustomizations": {
			// ═══════════════════════════════════════════════════════════
			// 🎨 ${THEME_NAME}
			// Generated with the vsGen design flow (https://github.io/XAOSTECH/design-tools) using pastel (https://github.com/sharkdp/pastel)
			// Primary:   ${PRIMARY_COLOUR} (${primary_hex})
			// Secondary: ${SECONDARY_COLOUR} (${secondary_hex})
			// Tertiary:  ${TERTIARY_COLOUR} (${tertiary_hex})
			// Variation: ${VARIATION}
			// ═══════════════════════════════════════════════════════════

			// ── Global Foreground & Borders ──
			"foreground": "${fg_main}",
			"disabledForeground": "${fg_muted}80",
			"descriptionForeground": "${fg_soft}",
			"errorForeground": "#ff5555",
			"icon.foreground": "${fg_soft}",
			"focusBorder": "${accent_primary}80",
			"contrastBorder": "${accent_primary}30",
			"contrastActiveBorder": "${accent_primary}60",
			"selection.background": "${accent_primary}40",
			"widget.shadow": "${bg_dark}80",
			"widget.border": "${accent_primary}30",
			"sash.hoverBorder": "${accent_primary}",

			// ── Text Colours ──
			"textLink.foreground": "${accent_tertiary}",
			"textLink.activeForeground": "${t1}",
			"textBlockQuote.background": "${bg_sidebar}",
			"textBlockQuote.border": "${accent_primary}40",
			"textCodeBlock.background": "${bg_panel}",
			"textPreformat.foreground": "${s2}",
			"textPreformat.background": "${bg_panel}",
			"textSeparator.foreground": "${p2}30",

			// ── Editor Background ──
			"editor.background": "${bg_main}",
			"editor.foreground": "${fg_main}",
			"editorLineNumber.foreground": "${p2}",
			"editorLineNumber.activeForeground": "${accent_secondary}",
			"editorLineNumber.dimmedForeground": "${p2}60",
			"editorCursor.foreground": "${s1}",
			"editorCursor.background": "${bg_dark}",
			"editor.selectionBackground": "${accent_primary}50",
			"editor.selectionForeground": "${fg_main}",
			"editor.inactiveSelectionBackground": "${accent_primary}30",
			"editor.selectionHighlightBackground": "${fg_soft}30",
			"editor.selectionHighlightBorder": "${fg_soft}50",
			"editor.wordHighlightBackground": "${s2}30",
			"editor.wordHighlightBorder": "${s2}50",
			"editor.wordHighlightStrongBackground": "${accent_tertiary}30",
			"editor.wordHighlightStrongBorder": "${accent_tertiary}50",
			"editor.wordHighlightTextBackground": "${bln1}20",
			"editor.wordHighlightTextBorder": "${bln1}40",
			"editor.findMatchBackground": "${accent_secondary}50",
			"editor.findMatchHighlightBackground": "${s3}40",
			"editor.findMatchBorder": "${accent_secondary}80",
			"editor.findMatchHighlightBorder": "${s3}60",
			"editor.findRangeHighlightBackground": "${accent_primary}15",
			"editor.lineHighlightBackground": "${bg_hover}60",
			"editor.lineHighlightBorder": "${accent_primary}15",
			"editor.rangeHighlightBackground": "${accent_tertiary}15",
			"editor.rangeHighlightBorder": "${accent_tertiary}20",
			"editor.symbolHighlightBackground": "${accent_tertiary}25",
			"editor.symbolHighlightBorder": "${accent_tertiary}40",
			"editor.foldBackground": "${bg_hover}40",
			"editor.hoverHighlightBackground": "${accent_primary}15",
			"editor.linkedEditingBackground": "${accent_primary}20",
			"editor.snippetTabstopHighlightBackground": "${accent_primary}20",
			"editor.snippetTabstopHighlightBorder": "${accent_primary}40",
			"editor.snippetFinalTabstopHighlightBackground": "${accent_secondary}20",
			"editor.snippetFinalTabstopHighlightBorder": "${accent_secondary}40",
			"editor.stackFrameHighlightBackground": "${accent_secondary}30",
			"editor.focusedStackFrameHighlightBackground": "${s2}30",
			"editorBracketMatch.background": "${accent_primary}30",
			"editorBracketMatch.border": "${accent_primary}",
			"editorIndentGuide.background1": "${p2}20",
			"editorIndentGuide.activeBackground1": "${accent_primary}",
			"editorRuler.foreground": "${p2}30",
			"editorWhitespace.foreground": "${p2}20",
			"editorLink.activeForeground": "${s1}",
			"editorCodeLens.foreground": "${fg_muted}80",
			"editorLightBulb.foreground": "${accent_secondary}",
			"editorLightBulbAutoFix.foreground": "${s2}",
			"editorLightBulbAi.foreground": "${accent_tertiary}",
			"editorGhostText.foreground": "${fg_muted}60",
			"editorGhostText.border": "${fg_muted}20",
			"editorStickyScroll.background": "${bg_main}",
			"editorStickyScrollHover.background": "${bg_hover}",
			"editorUnicodeHighlight.background": "${accent_tertiary}20",
			"editorUnicodeHighlight.border": "${accent_tertiary}",
			"editorUnnecessaryCode.opacity": "#000000aa",
			"editorUnnecessaryCode.border": "${fg_muted}40",

			// ── Bracket Pair Colourisation ──
			"editorBracketHighlight.foreground1": "${accent_primary}",
			"editorBracketHighlight.foreground2": "${accent_secondary}",
			"editorBracketHighlight.foreground3": "${accent_tertiary}",
			"editorBracketHighlight.foreground4": "${ana0}",
			"editorBracketHighlight.foreground5": "${tri0}",
			"editorBracketHighlight.foreground6": "${bln0}",
			"editorBracketHighlight.unexpectedBracket.foreground": "#ff5555",
			"editorBracketPairGuide.background1": "${accent_primary}30",
			"editorBracketPairGuide.background2": "${accent_secondary}30",
			"editorBracketPairGuide.background3": "${accent_tertiary}30",
			"editorBracketPairGuide.activeBackground1": "${accent_primary}60",
			"editorBracketPairGuide.activeBackground2": "${accent_secondary}60",
			"editorBracketPairGuide.activeBackground3": "${accent_tertiary}60",

			// ── Editor Gutter ──
			"editorGutter.background": "${bg_main}",
			"editorGutter.addedBackground": "${s2}cc",
			"editorGutter.modifiedBackground": "${accent_tertiary}cc",
			"editorGutter.deletedBackground": "${p1}cc",
			"editorGutter.commentRangeForeground": "${p3}60",
			"editorGutter.commentGlyphForeground": "${fg_muted}",
			"editorGutter.commentUnresolvedGlyphForeground": "${accent_secondary}",
			"editorGutter.foldingControlForeground": "${fg_soft}",

			// ── Editor Overview Ruler ──
			"editorOverviewRuler.background": "${bg_dark}",
			"editorOverviewRuler.border": "${bg_dark}",
			"editorOverviewRuler.findMatchForeground": "${accent_secondary}80",
			"editorOverviewRuler.rangeHighlightForeground": "${accent_tertiary}60",
			"editorOverviewRuler.selectionHighlightForeground": "${fg_soft}60",
			"editorOverviewRuler.wordHighlightForeground": "${accent_primary}60",
			"editorOverviewRuler.wordHighlightStrongForeground": "${accent_tertiary}80",
			"editorOverviewRuler.wordHighlightTextForeground": "${bln1}60",
			"editorOverviewRuler.modifiedForeground": "${accent_tertiary}cc",
			"editorOverviewRuler.addedForeground": "${s2}cc",
			"editorOverviewRuler.deletedForeground": "${p1}cc",
			"editorOverviewRuler.errorForeground": "#ff5555cc",
			"editorOverviewRuler.warningForeground": "${accent_secondary}cc",
			"editorOverviewRuler.infoForeground": "${accent_tertiary}cc",
			"editorOverviewRuler.bracketMatchForeground": "${accent_primary}80",

			// ── Editor Widgets ──
			"editorWidget.background": "${bg_widget}",
			"editorWidget.border": "${accent_primary}60",
			"editorWidget.foreground": "${fg_soft}",
			"editorWidget.resizeBorder": "${accent_primary}",
			"editorSuggestWidget.background": "${bg_widget}",
			"editorSuggestWidget.border": "${accent_primary}40",
			"editorSuggestWidget.foreground": "${fg_main}",
			"editorSuggestWidget.highlightForeground": "${accent_secondary}",
			"editorSuggestWidget.selectedBackground": "${accent_primary}30",
			"editorSuggestWidget.selectedForeground": "${fg_main}",
			"editorSuggestWidget.selectedIconForeground": "${fg_main}",
			"editorSuggestWidget.focusHighlightForeground": "${s1}",
			"editorHoverWidget.background": "${bg_widget}",
			"editorHoverWidget.border": "${accent_primary}40",
			"editorHoverWidget.foreground": "${fg_main}",
			"editorHoverWidget.highlightForeground": "${accent_secondary}",
			"editorHoverWidget.statusBarBackground": "${bg_panel}",
			"editorMarkerNavigation.background": "${bg_widget}",
			"editorMarkerNavigationError.background": "#ff555530",
			"editorMarkerNavigationWarning.background": "${accent_secondary}30",
			"editorMarkerNavigationInfo.background": "${accent_tertiary}30",
			"editorMarkerNavigationError.headerBackground": "#ff555520",
			"editorMarkerNavigationWarning.headerBackground": "${accent_secondary}20",
			"editorMarkerNavigationInfo.headerBackground": "${accent_tertiary}20",

			// ── Error / Warning / Info ──
			"editorError.foreground": "#ff5555",
			"editorError.border": "#ff555530",
			"editorError.background": "#ff555510",
			"editorWarning.foreground": "${accent_secondary}",
			"editorWarning.border": "${accent_secondary}30",
			"editorWarning.background": "${accent_secondary}10",
			"editorInfo.foreground": "${accent_tertiary}",
			"editorInfo.border": "${accent_tertiary}30",
			"editorInfo.background": "${accent_tertiary}10",
			"editorHint.foreground": "${fg_muted}",
			"editorHint.border": "${fg_muted}30",
			"problemsErrorIcon.foreground": "#ff5555",
			"problemsWarningIcon.foreground": "${accent_secondary}",
			"problemsInfoIcon.foreground": "${accent_tertiary}",

			// ── Sidebar & Activity Bar ──
			"sideBar.background": "${bg_sidebar}",
			"sideBar.foreground": "${fg_soft}",
			"sideBar.border": "${bg_dark}",
			"sideBar.dropBackground": "${accent_primary}30",
			"sideBarTitle.foreground": "${s1}",
			"sideBarSectionHeader.background": "${bg_panel}",
			"sideBarSectionHeader.foreground": "${p1}",
			"sideBarSectionHeader.border": "${bg_dark}",
			"activityBar.background": "${bg_dark}",
			"activityBar.foreground": "${accent_primary}",
			"activityBar.inactiveForeground": "${p2}60",
			"activityBar.border": "${bg_dark}",
			"activityBar.activeBorder": "${accent_primary}",
			"activityBar.activeBackground": "${accent_primary}15",
			"activityBar.dropBorder": "${accent_primary}",
			"activityBarBadge.background": "${accent_secondary}",
			"activityBarBadge.foreground": "${bg_dark}",

			// ── Badges ──
			"badge.background": "${badge_bg}",
			"badge.foreground": "${badge_fg}",

			// ── Title Bar ──
			"titleBar.activeBackground": "${bg_dark}",
			"titleBar.activeForeground": "${fg_soft}",
			"titleBar.inactiveBackground": "${bg_main}",
			"titleBar.inactiveForeground": "${p2}80",
			"titleBar.border": "${bg_dark}",

			// ── Menu Bar ──
			"menubar.selectionForeground": "${fg_main}",
			"menubar.selectionBackground": "${accent_primary}30",
			"menubar.selectionBorder": "${accent_primary}40",
			"menu.foreground": "${fg_soft}",
			"menu.background": "${bg_sidebar}",
			"menu.selectionForeground": "${fg_main}",
			"menu.selectionBackground": "${accent_primary}40",
			"menu.selectionBorder": "${accent_primary}50",
			"menu.separatorBackground": "${p2}30",
			"menu.border": "${bg_dark}",

			// ── Status Bar ──
			"statusBar.background": "${accent_primary}",
			"statusBar.foreground": "${bg_dark}",
			"statusBar.border": "${accent_primary}",
			"statusBar.debuggingBackground": "${accent_secondary}",
			"statusBar.debuggingForeground": "${bg_dark}",
			"statusBar.debuggingBorder": "${accent_secondary}",
			"statusBar.noFolderBackground": "${p3}",
			"statusBar.noFolderForeground": "${bg_dark}",
			"statusBar.noFolderBorder": "${p3}",
			"statusBar.focusBorder": "${accent_primary}80",
			"statusBarItem.hoverBackground": "${p1}",
			"statusBarItem.hoverForeground": "${bg_dark}",
			"statusBarItem.activeBackground": "${p2}",
			"statusBarItem.prominentBackground": "${s1}",
			"statusBarItem.prominentForeground": "${bg_dark}",
			"statusBarItem.prominentHoverBackground": "${s2}",
			"statusBarItem.remoteBackground": "${accent_tertiary}",
			"statusBarItem.remoteForeground": "${bg_dark}",
			"statusBarItem.remoteHoverBackground": "${t1}",
			"statusBarItem.errorBackground": "#ff5555",
			"statusBarItem.errorForeground": "#ffffff",
			"statusBarItem.errorHoverBackground": "#ff3333",
			"statusBarItem.warningBackground": "${accent_secondary}",
			"statusBarItem.warningForeground": "${bg_dark}",
			"statusBarItem.warningHoverBackground": "${s1}",
			"statusBarItem.compactHoverBackground": "${accent_primary}30",
			"statusBarItem.focusBorder": "${bg_dark}80",
			"statusBarItem.offlineBackground": "${fg_muted}",
			"statusBarItem.offlineForeground": "${bg_dark}",

			// ── Tabs ──
			"tab.activeBackground": "${bg_panel}",
			"tab.activeForeground": "${s5}",
			"tab.inactiveBackground": "${bg_main}",
			"tab.inactiveForeground": "${fg_soft}80",
			"tab.border": "${bg_dark}",
			"tab.activeBorderTop": "${accent_primary}",
			"tab.activeModifiedBorder": "${accent_secondary}",
			"tab.inactiveModifiedBorder": "${accent_secondary}60",
			"tab.unfocusedActiveModifiedBorder": "${accent_secondary}40",
			"tab.unfocusedInactiveModifiedBorder": "${accent_secondary}30",
			"tab.hoverBackground": "${bg_hover}",
			"tab.hoverForeground": "${fg_main}",
			"tab.hoverBorder": "${accent_primary}40",
			"tab.unfocusedHoverBackground": "${bg_hover}80",
			"tab.unfocusedHoverForeground": "${fg_soft}",
			"tab.unfocusedActiveBackground": "${bg_main}",
			"tab.unfocusedActiveForeground": "${fg_soft}",
			"tab.unfocusedInactiveBackground": "${bg_main}",
			"tab.unfocusedInactiveForeground": "${p3}80",
			"tab.lastPinnedBorder": "${accent_tertiary}50",
			"tab.dragAndDropBorder": "${accent_primary}60",
			"editorGroupHeader.tabsBackground": "${bg_dark}",
			"editorGroupHeader.tabsBorder": "${bg_dark}",
			"editorGroupHeader.noTabsBackground": "${bg_dark}",
			"editorGroupHeader.border": "${bg_dark}",
			"editorGroup.border": "${accent_primary}30",
			"editorGroup.dropBackground": "${accent_primary}30",
			"editorGroup.emptyBackground": "${bg_main}",
			"editorGroup.focusedEmptyBorder": "${accent_primary}30",
			"editorGroup.dropIntoPromptForeground": "${fg_main}",
			"editorGroup.dropIntoPromptBackground": "${bg_widget}",
			"editorGroup.dropIntoPromptBorder": "${accent_primary}40",

			// ── Panel (Terminal, Output, etc.) ──
			"panel.background": "${bg_main}",
			"panel.border": "${accent_primary}50",
			"panel.dropBorder": "${accent_primary}",
			"panelTitle.activeBorder": "${accent_secondary}",
			"panelTitle.activeForeground": "${s1}",
			"panelTitle.inactiveForeground": "${fg_soft}80",
			"panelInput.border": "${accent_primary}40",
			"panelSection.border": "${accent_primary}30",
			"panelSection.dropBackground": "${accent_primary}30",
			"panelSectionHeader.background": "${bg_panel}",
			"panelSectionHeader.foreground": "${fg_soft}",
			"panelSectionHeader.border": "${accent_primary}20",
			"terminal.foreground": "${fg_main}",
			"terminal.background": "${bg_main}",
			"terminal.selectionBackground": "${accent_primary}40",
			"terminal.selectionForeground": "${fg_main}",
			"terminal.findMatchBackground": "${accent_secondary}50",
			"terminal.findMatchHighlightBackground": "${s3}40",
			"terminal.findMatchBorder": "${accent_secondary}80",
			"terminal.findMatchHighlightBorder": "${s3}60",
			"terminal.hoverHighlightBackground": "${accent_primary}15",
			"terminal.inactiveSelectionBackground": "${accent_primary}20",
			"terminal.ansiBlack": "${bg_dark}",
			"terminal.ansiRed": "${p1}",
			"terminal.ansiGreen": "${s2}",
			"terminal.ansiYellow": "${accent_secondary}",
			"terminal.ansiBlue": "${accent_tertiary}",
			"terminal.ansiMagenta": "${accent_primary}",
			"terminal.ansiCyan": "${tri0}",
			"terminal.ansiWhite": "${fg_main}",
			"terminal.ansiBrightBlack": "${p2}",
			"terminal.ansiBrightRed": "${ana1}",
			"terminal.ansiBrightGreen": "${s4}",
			"terminal.ansiBrightYellow": "${bln1}",
			"terminal.ansiBrightBlue": "${tri1}",
			"terminal.ansiBrightMagenta": "${ana0}",
			"terminal.ansiBrightCyan": "${t4}",
			"terminal.ansiBrightWhite": "#ffffff",
			"terminal.border": "${accent_primary}50",
			"terminal.tab.activeBorder": "${accent_primary}",
			"terminalCursor.foreground": "${s1}",
			"terminalCursor.background": "${bg_dark}",
			"terminalCommandDecoration.defaultBackground": "${fg_muted}",
			"terminalCommandDecoration.successBackground": "${s2}",
			"terminalCommandDecoration.errorBackground": "#ff5555",
			"terminalOverviewRuler.cursorForeground": "${s1}",
			"terminalOverviewRuler.findMatchForeground": "${accent_secondary}80",

			// ── Lists & Trees ──
			"list.activeSelectionBackground": "${accent_primary}50",
			"list.activeSelectionForeground": "${s5}",
			"list.activeSelectionIconForeground": "${s5}",
			"list.dropBackground": "${accent_primary}30",
			"list.focusOutline": "${accent_primary}80",
			"list.focusAndSelectionOutline": "${accent_primary}",
			"list.hoverBackground": "${p2}20",
			"list.hoverForeground": "${fg_main}",
			"list.focusBackground": "${accent_secondary}30",
			"list.focusForeground": "${fg_main}",
			"list.focusHighlightForeground": "${accent_secondary}",
			"list.highlightForeground": "${accent_secondary}",
			"list.inactiveSelectionBackground": "${fg_soft}20",
			"list.inactiveSelectionForeground": "${fg_soft}",
			"list.inactiveSelectionIconForeground": "${fg_soft}",
			"list.inactiveFocusBackground": "${accent_primary}15",
			"list.inactiveFocusOutline": "${accent_primary}40",
			"list.invalidItemForeground": "#ff5555",
			"list.errorForeground": "#ff5555",
			"list.warningForeground": "${accent_secondary}",
			"list.deemphasizedForeground": "${fg_muted}",
			"list.filterMatchBackground": "${accent_secondary}30",
			"list.filterMatchBorder": "${accent_secondary}60",
			"listFilterWidget.background": "${bg_widget}",
			"listFilterWidget.outline": "${accent_primary}",
			"listFilterWidget.noMatchesOutline": "#ff5555",
			"listFilterWidget.shadow": "${bg_dark}80",
			"tree.indentGuidesStroke": "${p2}40",
			"tree.inactiveIndentGuidesStroke": "${p2}20",
			"tree.tableColumnsBorder": "${p2}20",
			"tree.tableOddRowsBackground": "${bg_hover}20",

			// ── Input & Dropdowns ──
			"input.background": "${bg_input}",
			"input.foreground": "${fg_main}",
			"input.border": "${accent_primary}50",
			"input.placeholderForeground": "${fg_soft}60",
			"inputOption.activeBorder": "${accent_secondary}",
			"inputOption.activeBackground": "${accent_secondary}30",
			"inputOption.activeForeground": "${fg_main}",
			"inputOption.hoverBackground": "${accent_primary}20",
			"inputValidation.errorBackground": "#ff555520",
			"inputValidation.errorForeground": "#ff5555",
			"inputValidation.errorBorder": "#ff5555",
			"inputValidation.warningBackground": "${accent_secondary}20",
			"inputValidation.warningForeground": "${accent_secondary}",
			"inputValidation.warningBorder": "${accent_secondary}",
			"inputValidation.infoBackground": "${accent_tertiary}20",
			"inputValidation.infoForeground": "${accent_tertiary}",
			"inputValidation.infoBorder": "${accent_tertiary}",
			"dropdown.background": "${bg_input}",
			"dropdown.border": "${accent_primary}50",
			"dropdown.foreground": "${fg_soft}",
			"dropdown.listBackground": "${bg_sidebar}",

			// ── Buttons ──
			"button.background": "${accent_primary}",
			"button.foreground": "${bg_dark}",
			"button.hoverBackground": "${p1}",
			"button.border": "${accent_primary}60",
			"button.secondaryBackground": "${accent_secondary}",
			"button.secondaryForeground": "${bg_dark}",
			"button.secondaryHoverBackground": "${s1}",
			"button.separator": "${bg_dark}40",
			"checkbox.background": "${bg_input}",
			"checkbox.foreground": "${fg_main}",
			"checkbox.border": "${accent_primary}50",
			"checkbox.selectBackground": "${bg_input}",
			"checkbox.selectBorder": "${accent_primary}80",
			"radio.activeBackground": "${accent_primary}",
			"radio.activeForeground": "${bg_dark}",
			"radio.activeBorder": "${accent_primary}",
			"radio.inactiveBackground": "${bg_input}",
			"radio.inactiveForeground": "${fg_soft}",
			"radio.inactiveBorder": "${accent_primary}50",
			"radio.inactiveHoverBackground": "${accent_primary}20",

			// ── Scrollbar ──
			"scrollbar.shadow": "${bg_dark}50",
			"scrollbarSlider.background": "${accent_primary}30",
			"scrollbarSlider.hoverBackground": "${accent_primary}60",
			"scrollbarSlider.activeBackground": "${accent_primary}80",

			// ── Progress Bar ──
			"progressBar.background": "${accent_secondary}",

			// ── Notifications ──
			"notificationCenter.border": "${accent_primary}",
			"notificationCenterHeader.background": "${bg_panel}",
			"notificationCenterHeader.foreground": "${fg_soft}",
			"notificationToast.border": "${accent_primary}60",
			"notifications.background": "${bg_sidebar}",
			"notifications.foreground": "${fg_soft}",
			"notifications.border": "${accent_primary}30",
			"notificationLink.foreground": "${accent_tertiary}",
			"notificationsInfoIcon.foreground": "${s1}",
			"notificationsWarningIcon.foreground": "${accent_secondary}",
			"notificationsErrorIcon.foreground": "#ff5555",

			// ── Banner ──
			"banner.background": "${bg_panel}",
			"banner.foreground": "${fg_main}",
			"banner.iconForeground": "${accent_secondary}",

			// ── Git Decorations ──
			"gitDecoration.addedResourceForeground": "${s2}",
			"gitDecoration.modifiedResourceForeground": "${accent_tertiary}",
			"gitDecoration.deletedResourceForeground": "${p1}",
			"gitDecoration.renamedResourceForeground": "${bln2}",
			"gitDecoration.untrackedResourceForeground": "${ana2}",
			"gitDecoration.conflictingResourceForeground": "${accent_secondary}",
			"gitDecoration.ignoredResourceForeground": "${p2}60",
			"gitDecoration.stageModifiedResourceForeground": "${bln3}",
			"gitDecoration.stageDeletedResourceForeground": "${p1}",
			"gitDecoration.submoduleResourceForeground": "${tri0}",

			// ── SCM ──
			"scm.providerBorder": "${accent_primary}30",

			// ── Diff Editor ──
			"diffEditor.insertedTextBackground": "${s2}20",
			"diffEditor.removedTextBackground": "${p1}20",
			"diffEditor.insertedLineBackground": "${s2}15",
			"diffEditor.removedLineBackground": "${p1}15",
			"diffEditor.border": "${accent_primary}30",
			"diffEditor.diagonalFill": "${p2}20",
			"diffEditor.insertedTextBorder": "${s2}30",
			"diffEditor.removedTextBorder": "${p1}30",
			"diffEditor.unchangedRegionBackground": "${bg_hover}40",
			"diffEditor.unchangedRegionForeground": "${fg_muted}",
			"diffEditor.unchangedCodeBackground": "${bg_hover}20",
			"diffEditor.move.border": "${accent_tertiary}60",
			"diffEditorGutter.insertedLineBackground": "${s2}40",
			"diffEditorGutter.removedLineBackground": "${p1}40",
			"diffEditorOverview.insertedForeground": "${s2}80",
			"diffEditorOverview.removedForeground": "${p1}80",

			// ── Merge Conflict ──
			"merge.currentHeaderBackground": "${accent_primary}40",
			"merge.currentContentBackground": "${accent_primary}20",
			"merge.incomingHeaderBackground": "${accent_tertiary}40",
			"merge.incomingContentBackground": "${accent_tertiary}20",
			"merge.commonHeaderBackground": "${fg_muted}30",
			"merge.commonContentBackground": "${fg_muted}15",
			"merge.border": "${fg_soft}30",
			"mergeEditor.change.background": "${accent_tertiary}15",
			"mergeEditor.change.word.background": "${accent_tertiary}30",
			"mergeEditor.conflict.unhandledUnfocused.border": "${accent_secondary}40",
			"mergeEditor.conflict.unhandledFocused.border": "${accent_secondary}",
			"mergeEditor.conflict.handledUnfocused.border": "${s2}40",
			"mergeEditor.conflict.handledFocused.border": "${s2}",
			"mergeEditor.conflict.handled.minimapOverViewRuler": "${s2}",
			"mergeEditor.conflict.unhandled.minimapOverViewRuler": "${accent_secondary}",
			"mergeEditor.conflictingLines.background": "${accent_secondary}15",

			// ── Minimap ──
			"minimap.background": "${bg_dark}",
			"minimap.foregroundOpacity": "#000000cc",
			"minimap.selectionHighlight": "${accent_primary}80",
			"minimap.findMatchHighlight": "${accent_secondary}80",
			"minimap.errorHighlight": "#ff555580",
			"minimap.warningHighlight": "${accent_secondary}80",
			"minimap.selectionOccurrenceHighlight": "${fg_soft}40",
			"minimap.infoHighlight": "${accent_tertiary}80",
			"minimapSlider.background": "${accent_primary}20",
			"minimapSlider.hoverBackground": "${accent_primary}40",
			"minimapSlider.activeBackground": "${accent_primary}60",
			"minimapGutter.addedBackground": "${s2}cc",
			"minimapGutter.modifiedBackground": "${accent_tertiary}cc",
			"minimapGutter.deletedBackground": "${p1}cc",

			// ── Peek View ──
			"peekView.border": "${accent_primary}",
			"peekViewEditor.background": "${bg_main}",
			"peekViewEditor.matchHighlightBackground": "${accent_secondary}50",
			"peekViewEditor.matchHighlightBorder": "${accent_secondary}60",
			"peekViewEditorGutter.background": "${bg_main}",
			"peekViewEditorStickyScroll.background": "${bg_main}",
			"peekViewResult.background": "${bg_sidebar}",
			"peekViewResult.fileForeground": "${fg_main}",
			"peekViewResult.lineForeground": "${fg_soft}",
			"peekViewResult.matchHighlightBackground": "${accent_secondary}40",
			"peekViewResult.selectionBackground": "${accent_primary}30",
			"peekViewResult.selectionForeground": "${fg_main}",
			"peekViewTitle.background": "${bg_dark}",
			"peekViewTitleLabel.foreground": "${fg_soft}",
			"peekViewTitleDescription.foreground": "${fg_muted}",

			// ── Breadcrumbs ──
			"breadcrumb.foreground": "${fg_soft}",
			"breadcrumb.focusForeground": "${s1}",
			"breadcrumb.activeSelectionForeground": "${accent_primary}",
			"breadcrumb.background": "${bg_main}",
			"breadcrumbPicker.background": "${bg_widget}",

			// ── Welcome Page ──
			"welcomePage.tileBackground": "${bg_sidebar}",
			"welcomePage.tileBorder": "${accent_primary}30",
			"welcomePage.tileHoverBackground": "${bg_hover}",
			"welcomePage.progress.foreground": "${accent_secondary}",
			"welcomePage.progress.background": "${bg_dark}",
			"walkThrough.embeddedEditorBackground": "${bg_main}",
			"walkthrough.stepTitle.foreground": "${fg_main}",

			// ── Settings Editor ──
			"settings.headerForeground": "${fg_main}",
			"settings.modifiedItemIndicator": "${accent_secondary}",
			"settings.focusedRowBackground": "${accent_primary}10",
			"settings.focusedRowBorder": "${accent_primary}30",
			"settings.rowHoverBackground": "${bg_hover}30",
			"settings.textInputBackground": "${bg_input}",
			"settings.textInputForeground": "${fg_main}",
			"settings.textInputBorder": "${accent_primary}40",
			"settings.checkboxBackground": "${bg_input}",
			"settings.checkboxForeground": "${fg_main}",
			"settings.checkboxBorder": "${accent_primary}40",
			"settings.dropdownBackground": "${bg_input}",
			"settings.dropdownForeground": "${fg_soft}",
			"settings.dropdownBorder": "${accent_primary}40",
			"settings.dropdownListBorder": "${accent_primary}50",
			"settings.numberInputBackground": "${bg_input}",
			"settings.numberInputForeground": "${fg_main}",
			"settings.numberInputBorder": "${accent_primary}40",
			"settings.sashBorder": "${accent_primary}30",

			// ── Debug ──
			"debugToolBar.background": "${bg_widget}",
			"debugToolBar.border": "${accent_secondary}60",
			"debugExceptionWidget.background": "${bg_widget}",
			"debugExceptionWidget.border": "#ff555580",
			"debugTokenExpression.name": "${accent_tertiary}",
			"debugTokenExpression.value": "${s2}",
			"debugTokenExpression.string": "${s3}",
			"debugTokenExpression.number": "${s1}",
			"debugTokenExpression.boolean": "${accent_primary}",
			"debugTokenExpression.error": "#ff5555",
			"debugView.exceptionLabelForeground": "#ffffff",
			"debugView.exceptionLabelBackground": "#ff555580",
			"debugView.stateLabelForeground": "${fg_main}",
			"debugView.stateLabelBackground": "${accent_primary}30",
			"debugView.valueChangedHighlight": "${accent_secondary}40",
			"debugIcon.breakpointForeground": "${accent_primary}",
			"debugIcon.breakpointDisabledForeground": "${p2}60",
			"debugIcon.breakpointUnverifiedForeground": "${fg_muted}",
			"debugIcon.breakpointCurrentStackframeForeground": "${accent_secondary}",
			"debugIcon.breakpointStackframeForeground": "${fg_muted}",
			"debugIcon.startForeground": "${s2}",
			"debugIcon.pauseForeground": "${accent_secondary}",
			"debugIcon.stopForeground": "#ff5555",
			"debugIcon.disconnectForeground": "#ff5555",
			"debugIcon.restartForeground": "${s2}",
			"debugIcon.stepOverForeground": "${accent_tertiary}",
			"debugIcon.stepIntoForeground": "${accent_tertiary}",
			"debugIcon.stepOutForeground": "${accent_tertiary}",
			"debugIcon.stepBackForeground": "${accent_tertiary}",
			"debugIcon.continueForeground": "${s2}",
			"debugConsole.infoForeground": "${accent_tertiary}",
			"debugConsole.warningForeground": "${accent_secondary}",
			"debugConsole.errorForeground": "#ff5555",
			"debugConsole.sourceForeground": "${fg_soft}",
			"debugConsoleInputIcon.foreground": "${accent_primary}",

			// ── Testing ──
			"testing.iconPassed": "${s2}",
			"testing.iconFailed": "#ff5555",
			"testing.iconErrored": "#ff5555",
			"testing.iconQueued": "${accent_tertiary}",
			"testing.iconUnset": "${p2}60",
			"testing.iconSkipped": "${fg_soft}60",
			"testing.runAction": "${s2}",
			"testing.peekBorder": "${accent_primary}",
			"testing.peekHeaderBackground": "${bg_panel}",
			"testing.message.error.decorationForeground": "#ff5555",
			"testing.message.error.lineBackground": "#ff555510",
			"testing.message.info.decorationForeground": "${accent_tertiary}",
			"testing.message.info.lineBackground": "${accent_tertiary}10",

			// ── Command Palette / Quick Input ──
			"quickInput.background": "${bg_widget}",
			"quickInput.foreground": "${fg_main}",
			"quickInputList.focusBackground": "${accent_primary}30",
			"quickInputList.focusForeground": "${fg_main}",
			"quickInputList.focusIconForeground": "${fg_main}",
			"quickInputTitle.background": "${bg_dark}",
			"pickerGroup.border": "${accent_primary}30",
			"pickerGroup.foreground": "${accent_secondary}",
			"commandCenter.foreground": "${fg_soft}",
			"commandCenter.background": "${bg_dark}",
			"commandCenter.border": "${accent_primary}30",
			"commandCenter.activeForeground": "${fg_main}",
			"commandCenter.activeBackground": "${bg_hover}",
			"commandCenter.activeBorder": "${accent_primary}60",
			"commandCenter.inactiveForeground": "${fg_muted}",
			"commandCenter.inactiveBorder": "${accent_primary}20",
			"commandCenter.debuggingBackground": "${accent_secondary}20",

			// ── Keybinding Label ──
			"keybindingLabel.background": "${accent_primary}20",
			"keybindingLabel.foreground": "${fg_main}",
			"keybindingLabel.border": "${accent_primary}40",
			"keybindingLabel.bottomBorder": "${accent_primary}60",

			// ── Inlay Hints ──
			"editorInlayHint.foreground": "${p3}90",
			"editorInlayHint.background": "${bg_hover}40",
			"editorInlayHint.typeForeground": "${t3}90",
			"editorInlayHint.typeBackground": "${bg_hover}30",
			"editorInlayHint.parameterForeground": "${s3}90",
			"editorInlayHint.parameterBackground": "${bg_hover}30",

			// ── Search Editor ──
			"searchEditor.findMatchBackground": "${accent_secondary}30",
			"searchEditor.findMatchBorder": "${accent_secondary}60",
			"searchEditor.textInputBorder": "${accent_primary}40",
			"search.resultsInfoForeground": "${fg_muted}",

			// ── Notebook ──
			"notebook.editorBackground": "${bg_main}",
			"notebook.cellBorderColor": "${accent_primary}30",
			"notebook.cellHoverBackground": "${bg_hover}30",
			"notebook.cellInsertionIndicator": "${accent_primary}",
			"notebook.cellStatusBarItemHoverBackground": "${accent_primary}20",
			"notebook.cellToolbarSeparator": "${p2}30",
			"notebook.focusedCellBorder": "${accent_primary}",
			"notebook.focusedEditorBorder": "${accent_primary}60",
			"notebook.inactiveFocusedCellBorder": "${accent_primary}40",
			"notebook.outputContainerBackgroundColor": "${bg_sidebar}",
			"notebook.outputContainerBorderColor": "${accent_primary}20",
			"notebook.selectedCellBackground": "${accent_primary}15",
			"notebook.selectedCellBorder": "${accent_primary}60",
			"notebook.symbolHighlightBackground": "${accent_tertiary}20",
			"notebookScrollbarSlider.background": "${accent_primary}30",
			"notebookScrollbarSlider.hoverBackground": "${accent_primary}60",
			"notebookScrollbarSlider.activeBackground": "${accent_primary}80",
			"notebookStatusRunningIcon.foreground": "${accent_secondary}",
			"notebookStatusSuccessIcon.foreground": "${s2}",
			"notebookStatusErrorIcon.foreground": "#ff5555",
			"notebookEditorOverviewRuler.runningCellForeground": "${accent_secondary}",

			// ── Charts ──
			"charts.foreground": "${fg_main}",
			"charts.lines": "${fg_soft}",
			"charts.red": "${p1}",
			"charts.blue": "${accent_tertiary}",
			"charts.yellow": "${accent_secondary}",
			"charts.orange": "${bln2}",
			"charts.green": "${s2}",
			"charts.purple": "${tri1}",

			// ── Extensions ──
			"extensionButton.prominentBackground": "${accent_primary}",
			"extensionButton.prominentForeground": "${bg_dark}",
			"extensionButton.prominentHoverBackground": "${p1}",
			"extensionButton.separator": "${bg_dark}40",
			"extensionBadge.remoteBackground": "${accent_tertiary}",
			"extensionBadge.remoteForeground": "${bg_dark}",
			"extensionIcon.starForeground": "${accent_secondary}",
			"extensionIcon.verifiedForeground": "${s2}",
			"extensionIcon.preReleaseForeground": "${accent_tertiary}",
			"extensionIcon.sponsorForeground": "${bln2}",

			// ── Ports ──
			"ports.iconRunningProcessForeground": "${s2}",

			// ── Profiles ──
			"profileBadge.background": "${accent_primary}",
			"profileBadge.foreground": "${bg_dark}"$(if [[ ${#VARIATION_COLOURS[@]} -gt 0 ]]; then echo ",

			// ── Variation Accents ──
			\"editorBracketPairGuide.activeBackground1\": \"${var0}60\",
			\"editorBracketPairGuide.activeBackground2\": \"${var1}60\",
			\"editorBracketPairGuide.activeBackground3\": \"${var2}60\",
			\"editorBracketPairGuide.activeBackground4\": \"${var3}60\""; fi)
		},
		"editor.tokenColorCustomizations": {
			"comments": "${p3}80",
			"strings": "${s2}",
			"keywords": "${accent_primary}",
			"numbers": "${s1}",
			"types": "${accent_tertiary}",
			"functions": "${bln0}",
			"variables": "${fg_main}",
			"textMateRules": [
				{
					"scope": ["entity.name.function", "support.function"],
					"settings": { "foreground": "${bln0}" }
				},
				{
					"scope": ["entity.name.type", "support.type", "support.class"],
					"settings": { "foreground": "${accent_tertiary}" }
				},
				{
					"scope": ["keyword", "storage.type", "storage.modifier"],
					"settings": { "foreground": "${accent_primary}" }
				},
				{
					"scope": ["keyword.operator"],
					"settings": { "foreground": "${p4}" }
				},
				{
					"scope": ["string", "string.quoted"],
					"settings": { "foreground": "${s2}" }
				},
				{
					"scope": ["string.regexp"],
					"settings": { "foreground": "${tri0}" }
				},
				{
					"scope": ["constant.numeric"],
					"settings": { "foreground": "${s1}" }
				},
				{
					"scope": ["constant.language", "support.constant"],
					"settings": { "foreground": "${accent_secondary}" }
				},
				{
					"scope": ["constant.character", "constant.other"],
					"settings": { "foreground": "${ana0}" }
				},
				{
					"scope": ["variable.parameter"],
					"settings": { "foreground": "${fg_main}", "fontStyle": "italic" }
				},
				{
					"scope": ["variable.other"],
					"settings": { "foreground": "${fg_main}" }
				},
				{
					"scope": ["variable.language"],
					"settings": { "foreground": "${accent_primary}", "fontStyle": "italic" }
				},
				{
					"scope": ["comment", "punctuation.definition.comment"],
					"settings": { "foreground": "${p3}80", "fontStyle": "italic" }
				},
				{
					"scope": ["entity.name.tag"],
					"settings": { "foreground": "${p1}" }
				},
				{
					"scope": ["entity.other.attribute-name"],
					"settings": { "foreground": "${s3}" }
				},
				{
					"scope": ["entity.other.attribute-name.id"],
					"settings": { "foreground": "${accent_tertiary}", "fontStyle": "bold" }
				},
				{
					"scope": ["entity.name.section", "entity.name.namespace"],
					"settings": { "foreground": "${tri1}" }
				},
				{
					"scope": ["punctuation", "meta.brace"],
					"settings": { "foreground": "${p4}" }
				},
				{
					"scope": ["punctuation.definition.tag"],
					"settings": { "foreground": "${p2}" }
				},
				{
					"scope": ["meta.decorator", "entity.name.function.decorator"],
					"settings": { "foreground": "${bln2}", "fontStyle": "italic" }
				},
				{
					"scope": ["meta.selector", "entity.other.attribute-name.class.css"],
					"settings": { "foreground": "${ana2}" }
				},
				{
					"scope": ["support.type.property-name.css"],
					"settings": { "foreground": "${bln3}" }
				},
				{
					"scope": ["markup.heading"],
					"settings": { "foreground": "${accent_primary}", "fontStyle": "bold" }
				},
				{
					"scope": ["markup.italic"],
					"settings": { "fontStyle": "italic" }
				},
				{
					"scope": ["markup.bold"],
					"settings": { "fontStyle": "bold" }
				},
				{
					"scope": ["markup.underline"],
					"settings": { "fontStyle": "underline" }
				},
				{
					"scope": ["markup.inline.raw"],
					"settings": { "foreground": "${s2}" }
				},
				{
					"scope": ["markup.deleted"],
					"settings": { "foreground": "${p1}" }
				},
				{
					"scope": ["markup.inserted"],
					"settings": { "foreground": "${s2}" }
				},
				{
					"scope": ["markup.changed"],
					"settings": { "foreground": "${accent_tertiary}" }
				},
				{
					"scope": ["markup.list"],
					"settings": { "foreground": "${accent_primary}" }
				},
				{
					"scope": ["markup.quote"],
					"settings": { "foreground": "${t3}", "fontStyle": "italic" }
				},
				{
					"scope": ["source.json support.type.property-name"],
					"settings": { "foreground": "${bln2}" }
				},
				{
					"scope": ["meta.embedded", "source.groovy.embedded"],
					"settings": { "foreground": "${fg_main}" }
				},
				{
					"scope": ["invalid", "invalid.illegal"],
					"settings": { "foreground": "#ff5555", "fontStyle": "strikethrough" }
				},
				{
					"scope": ["invalid.deprecated"],
					"settings": { "foreground": "${fg_muted}", "fontStyle": "strikethrough" }
				}$(if [[ "$USE_COMPLEMENTARY" == true ]] && [[ -n "$compl_highlight" ]]; then echo ",
				{
					\"scope\": [\"markup.heading\", \"entity.name.section\"],
					\"settings\": { \"foreground\": \"${compl_highlight}\" }
				},
				{
					\"scope\": [\"markup.bold\"],
					\"settings\": { \"foreground\": \"${compl_accent}\", \"fontStyle\": \"bold\" }
				}"; fi)$(if [[ ${#VARIATION_COLOURS[@]} -gt 0 ]]; then echo ",
				{
					\"scope\": [\"meta.object-literal.key\"],
					\"settings\": { \"foreground\": \"${var0}\" }
				},
				{
					\"scope\": [\"entity.name.method\"],
					\"settings\": { \"foreground\": \"${var1}\" }
				},
				{
					\"scope\": [\"support.variable\"],
					\"settings\": { \"foreground\": \"${var2}\" }
				}"; fi)
			]
		}
THEME_EOF
}
